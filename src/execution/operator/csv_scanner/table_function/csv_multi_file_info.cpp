...
double CSVFileScan::GetProgressInFile(ClientContext &context) {
	...
	if (StringUtil::CIEquals(manager->file_handle->compression_type, FILE_GZIP_COMPRESSION_TYPE) ||
	    StringUtil::CIEquals(manager->file_handle->compression_type, FILE_ZSTD_COMPRESSION_TYPE)) {
