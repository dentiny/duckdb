...
void PipelineExecutor::FinishProcessing(int32_t operator_idx) {
	...
	if (pipeline.GetSource()) {
		const annotated_lock_guard<annotated_mutex> guard {pipeline.source_state->lock};
		pipeline.source_state->PreventBlocking();
		pipeline.source_state->UnblockTasks();
	}
	if (pipeline.GetSink()) {
		const annotated_lock_guard<annotated_mutex> guard {pipeline.GetSink()->sink_state->lock};
