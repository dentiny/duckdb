...
BufferPool::EvictionResult BufferPool::EvictObjectCacheEntriesIfNeeded(MemoryTag tag, idx_t extra_memory, idx_t memory_limit) {
	...
	while (!object_cache->IsEmpty()) {
		object_cache->EvictToReduceMemory(extra_memory); // attempt to flush "extra_memory" number of bytes
		if (memory_usage.GetUsedMemory(MemoryUsageCaches::NO_FLUSH) <= memory_limit) {
			success = true;
			break;
		}
...
BufferPool::EvictionResult BufferPool::EvictBlocks(MemoryTag tag, idx_t extra_memory, idx_t memory_limit,
	...
	// Evicting buffer blocks is not enough to control overall memory consumption.
	return EvictObjectCacheEntriesIfNeeded(tag, extra_memory, memory_limit);
}
