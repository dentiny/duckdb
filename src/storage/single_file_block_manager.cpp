...
block_id_t SingleFileBlockManager::GetFreeBlockIdInternal(FreeBlockType type) {
	annotated_lock_guard<annotated_mutex> lock(single_file_block_lock);
	block_id_t block_id;
	if (!free_list.empty()) {
...
void SingleFileBlockManager::MarkBlockAsModified(block_id_t block_id) {
	annotated_unique_lock<annotated_mutex> lock(single_file_block_lock);
	D_ASSERT(block_id >= 0);
...
void SingleFileBlockManager::Truncate() {
	...
	annotated_lock_guard<annotated_mutex> guard(single_file_block_lock);
...
bool SingleFileBlockManager::AddFreeBlock(annotated_unique_lock<annotated_mutex> &lock, block_id_t block_id) {
	if (!lock.owns_lock()) {
