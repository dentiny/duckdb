	...
	static unique_ptr<FunctionData> MultiFileBind(ClientContext &context, TableFunctionBindInput &input,
		...
		for (auto &kv : input.named_parameters) {
			...
			if (loption == "allow_empty") {
				auto value = kv.second.GetOptionalValue<Value>().value_or(Value(kv.second.GetType()));
				multi_file_reader->ParseOption(loption, value, file_options, context);
		...
		for (auto &kv : input.named_parameters) {
			auto loption = StringUtil::Lower(kv.first.GetIdentifierName());
			auto value = kv.second.GetOptionalValue<Value>().value_or(Value(kv.second.GetType()));
			if (multi_file_reader->ParseOption(loption, value, file_options, context)) {
				continue;
			}
			if (interface->ParseOption(context, kv.first.GetIdentifierName(), value, file_options, *options)) {
				continue;
			}
