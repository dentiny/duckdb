...
void JSONReader::OpenJSONFile() {
	...
	if (!IsOpen()) {
		// Use CachingFileSystemWrapper for better performance with remote files and buffering
		auto caching_fs = CachingFileSystemWrapper::Get(context);
		auto cached_file_handle = caching_fs.OpenFile(file, FileFlags::FILE_FLAGS_READ | options.compression);
		// Wrap the cached handle in JSONFileHandle for JSON-specific read coordination and pipe support
		file_handle = make_uniq<JSONFileHandle>(context, std::move(cached_file_handle), BufferAllocator::Get(context));
	}
	Reset();
}
